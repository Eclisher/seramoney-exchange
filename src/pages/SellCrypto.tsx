...
export default function SellCrypto() {
  ...
  useEffect(() => {
    const fetchDailyLimits = async () => {
      try {
        ...
        const todaySellTransactions = (transactions as TxLite[]).filter(
          (tx) => {
  ...
  useEffect(() => {
    const loadAddresses = async () => {
      ...
      try {
        ...
        const addresses = await getWalletAddresses(selectedWalletId);
        setSelectedWalletAddresses(Array.isArray(addresses) ? addresses : []);
      } catch (error) {
  ...
  useEffect(() => {
    if (!crypto || !network || selectedWalletAddresses.length === 0) {
      return;
    }

    const matchingAddress = selectedWalletAddresses.find(
      (addr) => addr.crypto_id === crypto.id && addr.network === network,
    );

    if (matchingAddress) {
      setSelectedAddress(matchingAddress.address);
    }
  }, [crypto, network, selectedWalletAddresses]);
  ...
  const createTransactionIfNeeded = async () => {
    ...
    const response = await api.post("/transactions", {
      ...
      wallet_address: selectedAddress || null,
      notes: "",
    });

    const id = extractTransactionId(response.data);
    const reference =
      typeof (response.data as { data?: { reference?: unknown } })?.data
        ?.reference === "string"
        ? ((response.data as { data: { reference: string } }).data
            .reference as string)
        : null;
  ...
  const goNext = async () => {
    ...
    if (step === 3) {
      ...
      if (selectedWallet?.address && !selectedAddress) {
        toast({
          title: "Adresse requise",
          description: "Sélectionnez une adresse de réception.",
          variant: "destructive",
        });
        return;
      }
            ...
            <div className="p-6 rounded-2xl bg-card border border-border space-y-6 min-h-[320px]">
                ...
                <>
                  ...
                  <div className="p-4 rounded-xl bg-muted">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      ...
                      {crypto.sellRate.toLocaleString()} Ar
                    </p>
                  </div>
                </>
              )}
                ...
                <>
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Choisissez le wallet sur lequel vous souhaitez recevoir
                      les Ariary.
                    </p>
                    <UserWalletPicker
                      wallets={wallets}
                  ...
                  {selectedWallet && selectedWalletAddresses.length > 0 && (
                    <div className="space-y-2 pt-2 border-t border-border">
                      <Label>Adresse de réception</Label>
                      <p className="text-sm text-muted-foreground">
                        Adresses disponibles pour {crypto?.symbol} ({network})
                      </p>
                      <div className="space-y-2">
                        {selectedWalletAddresses
                          .filter((addr) => addr.network === network)
                          .map((addr) => (
                            <button
                              key={addr.id}
                              type="button"
                              onClick={() => setSelectedAddress(addr.address)}
                              className={`w-full text-left px-4 py-3 rounded-xl border-2 font-mono text-xs break-all transition-all ${
              ...
              {step === 5 && (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Après la transaction portefeuille, cliquez sur Suivant et
                    envoyez la preuve.
                  </p>
                </div>
              )}
            ...
            <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/20 flex gap-3">
              ...
              <div className="text-sm">
                ...
                <p className="text-muted-foreground mt-1">
                  Après validation, vous recevrez l&apos;adresse wallet pour
                  envoyer vos cryptos. Le paiement Mobile Money sera effectué
                  après confirmation.
                </p>
              </div>
            </div>
